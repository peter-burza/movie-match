...
export default function MovieCard(props) {
  ...
  const { original_title, overview, poster_path, id, release_date } =
    moviePreviewData || {};
  ...
  useEffect(() => {
    const topDiv = document.getElementById("parent")
    if (modalVisible) {
      topDiv.style.filter = 'blur(15px)'
    } else {
      topDiv.style.filter = 'blur(0px)'
    }
  }, [modalVisible])
    ...
    <div>
        ...
        (!movieDetails && !movieCredits && !movieReleaseDates && !movieContentLoading ? (
          ...
          >
            <img
              ...
              className="lg:h-146 xs:aspect-[1/1] xs:object-contain s:aspect-[6/4] lg:aspect-[2/3] xl:h-full border-gradient"
            />
            <div className="flex flex-col gap-10 pr-3 max-h-[calc(90vh-9rem)] lg:overflow-y-auto"> {/*flex flex-col gap-13 pr-5 overflow-y-auto max-h-[calc(90vh-9rem)]*/}
              ...
              <div className={trailerList?.length < 1 ? 'hidden' : ''}>
                ...
                <div id="trailer-container" className={`flex px-5 gap-5 m-3 scroll-container`}>
      ...
      <button onClick={() => { setModalVisible(true); }} class="relative xl:max-w-55 md:max-w-45 sm:max-w-40 max-w-35 group aspect-[12/15] group overflow-hidden rounded-xl place-items-center appearance-none cursor-pointer shadow-[0_2px_15px_3px_gray]">
        <img
          ...
          className="relative object-cover transition duration-300 group-hover:blur-sm group-active:blur-sm"
        />

        <div className="absolute inset-0 flex items-start lg:items-center pt-8 lg:pt-0 justify-center bg-black/50 opacity-0 group-hover:opacity-100 group-active:opacity-100 transition duration-300">
          <h2 className="text-white text-base sm:text-lg lg:text-xl font-semibold px-2">{original_title}</h2>
        </div>
      </button>
    </div>
  );
